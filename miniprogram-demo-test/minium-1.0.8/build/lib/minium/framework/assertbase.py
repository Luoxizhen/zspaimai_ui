...
class AssertBase(unittest.TestCase):
    ...
    def _miniSetUp(self):
        ...
        if not getattr(self, "__skip_mini_init__", None):  # 初始化output需要放在result.starttest中，考虑到不使用miniresult的情况，在case.setup中需要检查一下配置初始化
            self._startTest()
    ...
    def _get_error_info(self):
        ...
        if length == 1:
            sys_info = (
                self._outcome.errors[-1][-1]
            )
        elif length > 1:
            while length:
                length -= 1
                sys_info = (
                    self._outcome.errors[length][-1]
                )
    ...
    def _teardown_collect(self):
        ...
        if sys_info:
            ...
            self.results["error_value"] = str(e_value.args[0]) if len(e_value.args) > 0 else ""
        else:
