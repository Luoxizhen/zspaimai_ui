...
class Session(Process):
    ...
    """
        ...
        self.device_id = (
            conf.get("device_desire", {}).get("device_info", {}).get("udid", None)
        )
    ...
    def run(self) -> None:
        ...
        while True:
            try:
                ...
                logger.info(
                    f"case queue is empty, test complete, release {self.device_id}"
                )
                break
                ...
                ep_data = {}
                ep_data["TimeStamp"] = time.strftime(
    ...
    def run_case(tests):
        ...
        result = minium.MiniResult(mini_suite=mini_suite)
        tests.run(result)
