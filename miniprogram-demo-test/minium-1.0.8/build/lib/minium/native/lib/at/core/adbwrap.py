...
class AdbWrap(object):
    ...
    def get_current_process_appbrand(self):
        output = self.run_shell("top -m 5 -n 2 -d 2")
        # logger.error(output)
        lines = [line.strip() for line in output.split("\n")]
        for output2 in lines:
            if output2.find("com.tencent.mm:") != -1:
                if output2.find("com.tencent.mm:appbrand") != -1:
                    prop_index = output2.index("com.tencent.mm:appbrand")
                    pid = output2.split()[0]
                    return (output2[prop_index:].strip(), pid)
                else:
                    pid = output2.split()[0]
                    logger.info(pid)
                    output3 = self.run_shell("ps -ef | grep %s" % pid)
                    logger.error(output3)
                    r = re.compile(r"(com.tencent.mm:appbrand\d+)")
                    m = r.search(output3)
                    if m:
                        m_name = m.group(1)
                        return (m_name, pid)
    ...
    def get_current_view_appbrand(self):
        output = self.run_adb("shell dumpsys SurfaceFlinger --list")
        lines = [line.strip() for line in output.split('\n')]
        view = None
        for line in lines:
            if "com.tencent.mm/com.tencent.mm.plugin.appbrand.ui.AppBrandUI" in line:
                view = line
    ...
    def get_cpu_rate_new_c(self, pkg_name):
        outputs = self.run_shell("ps -c | grep %s" % (pkg_name))
        lines = outputs.split("\n")
        if len(lines) < 1:
            return "0"
        for line in lines:
            if line.count("shell") == 0 and line.count(pkg_name)!=0:
                # logger.error(line)
                ls = re.split(r"\s+", line)
                return ls[5]
        return "0"

    def get_cpu_rate_new_ef(self, pkg_name):
        outputs = self.run_shell("ps -ef | grep %s" % (pkg_name))
    ...
    def get_cpu_rate(self, *pkg_names):
        for pkg_name in pkg_names:
            ...
            for line in lines:
                ls = re.split(r"\s+", line)
                if ls.count("shell") == 0 and ls.count(pkg_name)!=0:
                    return ls[3]
        return "0"
        #dumpsys的方法经常获取到的值是0，不知道什么原因
