...
class QQMinium(MiniumObject):
        ...
        if self.project_path:
            if not os.path.exists(self.dev_tool_path):
                raise MiniConfigError(
                    "dev_tool_path: %s not exists" % self.dev_tool_path
                )
            if not os.path.exists(self.project_path):
                raise MiniConfigError("project_path: %s not exists" % self.project_path)
            if not os.path.isdir(self.project_path):
                raise MiniConfigError(
                    "project_path: %s is not director" % self.project_path
                )
            if self._is_windows:
            ...
            if status[-1] == "":
                self.logger.info(
                    "Dev tool is opening, but can't open project, quit now..."
                )
                ...
                status = self._do_shell(cmd_start, True)
                Exception(
                    "Please check MiniProgram project if config error or not"
                ) if status[-1] == "" else self.logger.info("Restart finish")
        else:
            status = [
                "Open project with automation enabled success  /Users/sherlock/svn/demo"
            ]
    ...
    def launch_dev_tool_with_login(self):
        if self._is_windows:
            cmd_template = (
                '"%s" -l --login-qr-output terminal --auto --auto %s --auto-port %s'
            )
        else:
            cmd_template = (
                "%s  -l --login-qr-output terminal --auto --auto %s --auto-port %s"
    ...
    def enable_remote_debug(self, path=None, use_push=None, connect_timeout=None):
        ...
        qrUrl = self.connection.send(
            "Tool.enableRemoteDebug", max_timeout=180
        ).result.qrUrl
    ...
    def shutdown(self):
        ...
        if status == 0:
            ...
            self.logger.warning(
                "QQ Minium Test complete, but dev tool has not quit yet"
            )
