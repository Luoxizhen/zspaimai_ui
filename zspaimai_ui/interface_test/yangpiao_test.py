...
def t_yangpiao1():
    ...
    dealTime_start = times.str_to_time("2022-10-31 00:00:00")
    dealTime_end = times.str_to_time("2022-11-7 00:00:00")
    '''每周更新数据'''
    while 1:
        ...
        if r.json()["data"]!=[]:
            ...
            for a_bid_info in bid_infos: #每个拍品的信息
                if dealTime_start <= a_bid_info["dealTime"] < dealTime_end :
                    ...
                    # 拍品的名称，编号，品相，落槌价，佣金比例，出价次数，，优惠折扣，成交时间
                    bids.append(a_bid_info.values())
            file_path = "/Users/yuanyuanhe/Desktop/竞拍分析/yangpiao/123版币11-7.csv"  #写入一个页面的内容
            with open(file_path, "a", encoding="utf-8") as f:
                csv_writer = csv.writer(f)
                if bids:
                    print("版别：{}, 类目：{}, 页面：{}" .format(rid,n,p))
