...
def get_native(cfg):
 ...
 if g_native is None:
  ...
  if cfg.platform!="ide" and not cfg.close_ide:
   ...
   path=mini.enable_remote_debug(use_push=cfg.use_push,connect_timeout=cfg.remote_connect_timeout)
   if not cfg.use_push:
    g_native.connect_weapp(path)
    mini.connection.wait_for(method="App.initialized")
 return g_native
def get_minium(cfg):
 global g_minium
 if g_minium is None:
